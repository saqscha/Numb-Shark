...
class App(ttk.Frame):
    ...
    # Die nachfolgende Funktion ist sehr lang, es ist immer besser, kurze Funktionen zu haben.
    # Daher teile deine setups in kleine Portionen auf, und rufe diese wiederum in eigenen Funktionen
    # auf. Das hilft dabei, den Code übersichtlicher zu gestalten
    ...
    def updateShark(self, j):
        #self.button_int = 0
        j = j+1
        # es macht keinen Sinn hier self.i zu verwenden, da du i nur als  lokalen Zählindex verwendest
        self.i = 1
        # Ist hier eine while Schleife wirklich notwendig?
        # while schleifen sollte man immer nur dann einsetzen, wenn kein for möglich ist, da eine while Schleife
        # gefährlich sein kann, wenn die Abbruchbedingung falsch ist, dann bekommt man einen infinite loop
        # Daher überlege nochmal, ob du das hier nicht vielleicht in eine for loop umwandeln kannst
        while self.i <= j:
            if j % self.i == 0:
                Teiler = j//self.i
                if Teiler != j:
                    ...
                    self.i = self.i+1
                    # Was willst du hier machen? Du erzeugst ein dictionary, das als values "None" hat
                    # und anschließend wandelst du es wieder in eine liste um, und hast damit dasselbe
        ...
        self.sharkScore = 0
        # ich glaube, die Schleife über die buttons ist nicht unbedingt nötig,
        # versuche das umzuschreiben, so dass du keine
        # Schleife über den buttons benötigst (hat mit dem zu tun was du in disableButtons verbessern kannst)
    ...
    def disableButtons(self, j, var):
        ...
        # Das ist ein typischer denkfehler und geht viel einfacher:
        # self.button[var].config(...
        # Wenn du überprüfst, ob i=var ist, dann ist im folgenden i=var und du kannst direkt var verwenden
    ...
    def createPlayground(self):
        ...
        for j in range(self.playgroundVal):
            ...
            # Was ist der Unterschied zwischen j und var?
            # Als Tipp -> Versuche immer sprechende Variable zu nehmen, da ansonsten der Grund für die Variable
            # später nicht mehr ersichtlich ist
            # (ich denke es sollte die Zahl auf dem button wiederspiegeln, dann würde ich z.B. numberOnButton als Name
            # verwenden)
